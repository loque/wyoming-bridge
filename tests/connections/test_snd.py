...
def tts_connection():
    ...
    conn = WyomingSndConnection(uri="mock://snd", on_target_event=on_target_event_mock)
    # Mock the parent's disconnect method
    conn._disconnect = AsyncMock()
    return conn

# Tests for is_type
def test_is_type_snd():
    assert WyomingSndConnection.is_type("snd") is True

def test_is_type_not_snd():
    assert WyomingSndConnection.is_type("asr") is False

def test_is_type_none():
    assert WyomingSndConnection.is_type(None) is False

def test_is_type_empty():
    assert WyomingSndConnection.is_type("") is False
